...
Aggregate.prototype.createTasks = function(service, owner, repo, userpaths) {
  var tasks = [];
  for (var type in userpaths) {
    for (var i in userpaths[type]) {
      var path = userpaths[type][i].replace('{owner}', owner).replace('{repo}', repo);
      var url  = service + path + '?per_page=100';
      var task = {
        url:   url,
        path:  path,
        type:  type,
...
Aggregate.prototype.execute = function(final_cb) {
  ...
  self.q = async.queue(function (task, asyncTaskCb) {
    self.rateLimiter(function (err, timeout) {
      if (err) {
        return final_cb(err);
      }
  ...
  var tasks = self.createTasks(self.config.service, self.config.owner, self.config.repo, self.config.userpaths);
